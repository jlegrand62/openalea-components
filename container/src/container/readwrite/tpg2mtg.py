...
#
################################################################################
"""This module allows to convert TemporalPropertyGraph to MTG."""
...
def tpg2mtg(graph, properties=['index'], root_ids=None, mtg=None,
            binary_sorter=None):
    """
    Convert a TemporalPropertyGraph class object into a MTG object.
    ...
    properties : list(str)
        list of properties to export in MTG
    root_ids : list, optional
        list of root ids (@t0) to build the MTG
    mtg : MTG, optional
        an existing mtg used to receive the graph properties
    binary_sorter : func, optional
        function to use as binary sorter to create missing binary divisions
    """
    if root_ids is None:
        root_ids = [i for i in graph.vertices() if
                    len(graph.parents(i)) == 0 and len(graph.children(i)) > 0]
...
def expert_sorter(graph, chtcids):
    """
    Binary sorter using manual definition of 'sub-lineage' as a graph property.
    ...
    -------
    siblings : list
        a recursive list indicating the binary lineage, ex. [A,[B,C]]
    """
...
def auto_binary_sorter(graph, chtcids):
    """
    Select "automatically" the best sorter given list of siblings.
    ...
    if len(chtcids) == 3:
        if sum([graph.vertex_property('L1').has_key(cid) for cid in
                chtcids]) == 3:
        ...
        warnings.warn(
            "Not able to restore binary graph for more than 4 children...")
...
def volume_sorter(graph, chtcids):
    ...
    ABC -> A & BC -> A & B & C , in recursive list writing: [A,[B,C]]
    ...
    volumes = dict(
        [(vid, graph.vertex_property('volume')[vid]) for vid in chtcids])
    ...
    if ref_ratio < 0.7:
        print "Warning: the volume_sorter might be wrong in creating the most probable binary lineage for {}".format(
            siblings)
        print "Selected volume ratio = {}, against: {}".format(
            round(ref_ratio, 3), [round(ratio, 3) for ratio in other_ratios])
        if sum([graph.vertex_property('L1').has_key(cid) for cid in
                chtcids]) == 3:
            ...
            if ep_sorted == siblings:
                ...
                print "Epidermis_surface_sorter does not return the same result: {}".format(
                    ep_sorted)
...
def epidermis_surface_sorter(graph, chtcids):
    ...
    assert len(chtcids) == 3
    assert sum(
        [graph.vertex_property('L1').has_key(cid) for cid in chtcids]) == 3
    ep_surf = dict(
        [(vid, graph.vertex_property('epidermis_surface')[vid]) for vid in
         chtcids])
    ...
    if ref_ratio < 0.7:
        print "Warning: the epidermis_surface_sorter might be wrong in creating the most probable binary lineage for {}".format(
            siblings)
        print "Selected surface ratio = {}, against: {}".format(
            round(ref_ratio, 3), [round(ratio, 3) for ratio in other_ratios])
        ...
        for cid in other_siblings:
            ...
            if vid < cid and extern_nei[vid] & extern_nei[cid] != set([]):
                ...
                if [vid, cid] in not_adjacent:
                    print "Warning: the topological_sorter might be wrong in creating the most probable binary lineage for {}".format(
                        adjacent)
